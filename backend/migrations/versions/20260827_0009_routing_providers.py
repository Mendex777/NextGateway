...
def upgrade():
    ...
    if "include_direct" not in columns:
        op.add_column(
            "proxy_groups",
            sa.Column("include_direct", sa.Boolean(), nullable=False, server_default=sa.false()),
        )
    if "include_reject" not in columns:
        op.add_column(
            "proxy_groups",
            sa.Column("include_reject", sa.Boolean(), nullable=False, server_default=sa.false()),
        )
    ...
    if "proxy_group_group_members" not in tables:
        op.create_table(
            ...
            sa.Column(
                "group_id",
                sa.String(36),
                sa.ForeignKey("proxy_groups.id", ondelete="CASCADE"),
                nullable=False,
            ),
            sa.Column(
                "member_group_id",
